...
with open(f'test/postProcessing/forcesIncompressible/{t_init}/forces.dat', 'r') as f:
    data = f.readlines()
    data = data[3:]
...
cp = (power/(energy))
eff = cp/0.593
# calculating the torque coefficient cm 
...
plt.savefig(f"test/monitoring/cp_{tsr:.1f}.png", dpi=600, bbox_inches="tight")
